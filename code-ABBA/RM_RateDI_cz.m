% 改编自子函数：RM_RateOverlap_cz.m   2022/03/22
% 考察神经元峰值放电率在两个session中的变化趋势
% 公式：（S2峰-S1峰）/（S2峰+S1峰），若值为正，则S2峰大；

function RateDI = RM_RateDI_cz(column1,column2,repetitions,cutoff,nonzero,KC) 
...
% Output:struct RateDI
%       RateDI.RateRatio: ratio of the rate overlap between column1 and comlun2
%       RateDI.mean: mean of RateRatio
%       RateDI.sem: SEM of RateRatio
%       RateDI.n: length of RateRatio
%       RateDI.shuffle_mean: mean of RateRatio from permutated data
%       RateDI.shuffle_ratio: RateRatio from permutated data
...
% 我改的：remove the NaN in the data and excluded data
ind0=find(~isnan(column1+column2)==1);
ind=intersect(ind0,KC);
column1_nonnan=column1(ind);
column2_nonnan=column2(ind);
...
% Output RateDI
RateDI = struct('RateRatio',RateRatio,'mean',RateRatio_mean,'n',RateRatio_n,'sem',RateRatio_sem,'shuffle_mean',shuffle_mean,'shuffle_ratio',shuffleratio);
...
ratio = (Y(index)-X(index)) ./ (Y(index)+X(index));
