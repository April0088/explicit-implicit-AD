function [PFCorr,stack1_active,stack2_active] = RM_SpatialCorr_cz_v2(stack1,stack2,repetitions,cutoff,nonzero,RC)
% 2021.7.5在RM_SpatialCorr_cz_v2.m的基础上修改
...
for Z=1:1:ncell
    MeanRates1(Z)=mean(nanmean(stack1(:,:,Z)));
    MeanRates2(Z)=mean(nanmean(stack2(:,:,Z)));
end

% calculate PF correlations
[PFCorr_mean, PFCorr_sem, PFCorr_n, PFCorr_active,PFCorr,stack1_active,stack2_active] = CalculatePFCorr_v2(stack1,stack2,cutoff,nonzero,RC);
...
% Output PFCorr
PFCorr = struct('PFCorrActive',PFCorr_active,'PFCorr',PFCorr,'mean',PFCorr_mean,'n',PFCorr_n,'sem',PFCorr_sem,'shuffle_mean',shuffle_mean,'shuffle_PFCorr',shufflePFCorr);

% =========== SubFunction to calculate PFCorr ===============
function [PFCorr_mean, PFCorr_sem, PFCorr_n, PFcorr_active,PFcorr,XX,YY] = CalculatePFCorr_v2(X,Y,cutoff,nonzero,RC)
...
%             [r,c] = find(~isnan(X(:,:,i))& ~isnan(Y(:,:,i))& (X(:,:,i) > nonzero | Y(:,:,i) > nonzero));
       if length(RC{i})>2
            ...
            c = RC{i}(:,2);
            % NaN firing rate in unvisited bins
            x=diag(X(r,c,i),0);  % x=X(r,c,i); returns matrix    从ratemap中提取出对应r、c的值
            y=diag(Y(r,c,i),0);
