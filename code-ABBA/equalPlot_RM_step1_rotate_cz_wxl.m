...
function [rateMaps,timeMaps,peakRate,mapAxis,sessions,F,pathCoord,timeStamps,numCells] ...
    = equalPlot_RM_step1_rotate_cz_wxl(TTlist,session_period,rorate_para,numsession,sLength,bins)
...
% load(strcat(currentdir,'/video_track.mat'));
for ii = 1:numsession
    % Get position data
    t=Process_Data_video{ii}(:,1)';
    x=Process_Data_video{ii}(:,2)';
    y=Process_Data_video{ii}(:,3)';
    ind = find(~isnan(x) & ~isnan(y));
    t = t(ind);
    x = x(ind);
    y = y(ind);
    vfs=25;  % ======= the sample frequency of video ========
...
for ii = 1:numsession  % == sessions numbers ==
    fprintf('%s%s\n','Reading data for session: ',mat2str(ii));
    ...
    dirFiles = dir(sessions{1});   % Files in directory
    ...
    S = loadSpikes(F,sessions{1},NaFile);
    spk = spikes_all{ii};
